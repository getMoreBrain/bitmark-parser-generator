@bitmark-standard/bmg
================

![Build & Test](https://github.com/bitmark-standard/bmg/actions/workflows/build-test.yml/badge.svg?branch=main)

Simple, typesafe enums in TypeScript, fully compatible with standard JavaScript.


## Why @bitmark-standard/bmg?
...
@bitmark-standard/bmg provides an alternative that is:
...
```sh
yarn add @bitmark-standard/bmg
npm install --save @bitmark-standard/bmg
```

## Importing
`@bitmark-standard/bmg` can be used via ES Module import, CommonJS, or as a script import directly in the browser.

```ts
import { bmg, EnumType } from '@bitmark-standard/bmg';
```

```ts
const { bmg, EnumType } = require('@bitmark-standard/bmg');
```

```html
<script src="https://cdn.jsdelivr.net/npm/@bitmark-standard/bmg/dist/browser/bmg.min.js"></script>
<script
...
## Enum Declaration
Enums are declared as JavaScript objects wrapped with the `bmg.fromObject(enumObject)` function.
...
Enums can also be declared as JavaScript arrays using `bmg.fromArray(enumArray)`. These will be expanded to objects with the same keys as values, and they will behave as if they had be declared as such.

```ts
// String enum
const MyEnum = bmg({ // ALIAS: bmg.fromObject({
  node: 'node',
  chrome: 'chrome',
  safari: 'safari',
});
...
const MyEnumFromArray = bmg.fromArray([
  'node',
  'chrome',
  'safari',
]);
...
`@bitmark-standard/bmg` makes this easy with `<enum>.fromValue()` which returns a typed enum or undefined if the
data does not match an enum value.

```ts
const MyEnum = bmg.fromArray([
  'node',
  'chrome',
  'safari',
]);
...
The order of iteration is guaranteed unless an enum key can be converted to an integer. In this case it can be guaranteed by using `bmg.fromArray()` to initialise the enum, or by setting `EnumOptions.iterationKeys` to represent the desired iteration order.

```ts
// Ensure iteration order when using integer keys mixed with strings and creating
// the object with `bmg() / bmg.fromObject()
//
...
| Feature | `@bitmark-standard/enum` (numeric and string) | TypeScript `enum` (numeric) | TypeScript `enum` (string) |
| ------- | ------------------------------------- | --------------------------- | -------------------------- |
