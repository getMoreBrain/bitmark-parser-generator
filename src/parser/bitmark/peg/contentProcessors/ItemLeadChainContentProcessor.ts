...
function itemLeadChainContentProcessor(
  ...
  if (bitLevel === BitContentLevel.Chain) {
    itemLeadTagContentProcessor(context, bitType, bitLevel, tagsConfig, content, target);
  } else {
    buildItemLead(context, bitType, bitLevel, tagsConfig, content, target);
  }
}
...
function buildItemLead(
  ...
  const chainTags = context.bitContentProcessor(bitType, BitContentLevel.Chain, itemLeadConfig?.chain, chainContent);
