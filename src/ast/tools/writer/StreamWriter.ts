...
import { RegionMarkerFormatter, DefaultRegionMarkerFormatter } from './RegionMarkerFormatter';
...
export class StreamWriter implements TextWriter {
  ...
  public writeLine(value?: string): this {
    if (value != null) {
      if (this.noEndOfLine) this.stream.write(this.createIndentString() + value);
      else this.stream.write(this.createIndentString() + value + this.endOfLineString);

      this.incrementLoc(true);
    } else {
      if (!this.noEndOfLine) this.stream.write(this.endOfLineString);
  ...
  public writeLineIndented(value: string): this {
    const indentString = this.noIndent ? '' : this.indentString + this.createIndentString();
  ...
  public writeFile(path: string, encoding?: string): this {
    ...
    if (contents == null || contents.length <= 0) return this;
  ...
  public writeFileRegion(regionName: string, path: string, encoding?: string): boolean {
    ...
    const region = contents.substring(regionStartIndex + regionStartMarker.length, regionEndIndex);
  ...
  private createIndentString(): string {
    let result = '';
    if (this.noIndent) return result;
