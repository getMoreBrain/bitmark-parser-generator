...
const TEST_OUTPUT_DIR = path.resolve(__dirname, './results/text-bitmark-body-generator/output');
...
describe('text-bitmark-body-generator', () => {
  describe('JSON => Bitmark Text (body) => JSON', () => {
    ...
    allTestFiles.forEach((testFile: string) => {
      ...
      test(`${id}`, async () => {
        ...
        const text = textGenerator.generateSync(originalJson, TextFormat.bitmarkText, TextLocation.body);
